...
static void *get_arg_pointer(struct intr_frame *f, int i, int len, bool grow){
  void **p = (((void**)(f)->esp) + (i));
  check_pointer((uint32_t)f->esp, p, grow);
  check_pointer((uint32_t)f->esp, (char*)p + 3, grow);
  check_pointer((uint32_t)f->esp, *p, grow);
  //check_pointer((char*)*p + 3);

  char *ret = (char*)*p;
  check_pointer((uint32_t)f->esp, ret, grow);
  if(len == -1){
    for(;*ret;check_pointer((uint32_t)f->esp, ++ret, grow));
  }else{
    check_pointer((uint32_t)f->esp, ret + len, grow);
    for(;len >= 0; len--, check_pointer((uint32_t)f->esp, ret++, grow));
  }
